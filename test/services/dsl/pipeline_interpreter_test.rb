...
class Dsl::PipelineInterpreterTest < ActiveSupport::TestCase
  ...
  test "extraction_empty is true when all xpath-like steps are empty" do
    steps = [
    ...
    assert_equal true, Dsl::PipelineInterpreter.new(steps).metrics[:extraction_empty]
  end
  ...
  test "failure_step uses first data-loss step when no error exists" do
    steps = [
      { step: 1, type: "xpath", output: ["a"] },
      { step: 2, type: "ruby", output: [] },
  ...
  test "failure_step returns actual non-sequential step number" do
    steps = [
      { step: 10, type: "xpath", output: ["a"] },
      { step: 42, type: "ruby", output: [] },
      { step: 90, type: "ruby", output: ["recovered"] }
    ]

    assert_equal 42, Dsl::PipelineInterpreter.new(steps).metrics[:failure_step]
  end
  ...
  test "xpath all blank means extraction attempted and empty" do
    steps = [
  ...
  test "failure_step points to first loss when multiple losses exist and no errors" do
    steps = [
    ...
    assert_equal 4, Dsl::PipelineInterpreter.new(steps).metrics[:failure_step]
  end
