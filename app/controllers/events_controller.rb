class EventsController < ApplicationController
  ...
  def index
    ...
    titles = Statement.joins(source: [:property, :website]).joins(:webpage).where({sources:{selected: true, properties:{label: "Title", rdfs_class: 1},websites:  {seedurl: params[:seedurl]}  }  }  ).pluck(:rdf_uri,:cache, "sources.language")
    titles_hash = titles.map {|title| [title[0],title[1]] if !title[1].blank? }.to_h
    photos_hash = Statement.joins(source: [:property, :website]).joins(:webpage).where({sources:{selected: true, properties:{label: "Photo", rdfs_class: 1},websites:  {seedurl:  params[:seedurl]}  }  }  ).pluck(:rdf_uri, :cache).to_h


    #merge lists
    photos_hash.each do |photo|
        @events << {rdf_uri: photo[0], statements_status: {}, photo: photo[1], title: titles_hash[photo[0]]  }
    end
    #uris_with_problems = Statement.joins(webpage: :website).where(webpages:{websites: {seedurl: "fass-ca"}}).where(status: "problem").pluck(:rdf_uri).uniq
