...
class DbCleanupJob < ApplicationJob
  ...
  def perform()
    ...
    Webpage.where(website: websites).where("archive_date < ?", Date.today - 2.months).destroy_all # destroy_all will delete all statements per webpage. Destroys the records by instantiating each record and calling its #destroy method. Each object's callbacks are executed (including :dependent association options).
  end
end