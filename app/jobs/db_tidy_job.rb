...
class DbTidyJob < ApplicationJob
  queue_as :default
  def perform()
    Message.where("updated_at < ?", Time.zone.today - 1.month).delete_all  # delete_all does not delete dependancies
